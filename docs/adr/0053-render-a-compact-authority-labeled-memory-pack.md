...
Effective priority tier, primary category, score components, evidence state, `startup: always` participation, and omission detail remain in the Injection Receipt. In particular, `startup: always` does not appear as an authority upgrade. Human-authored and Agent-derived content remain visibly distinct. Identity-only items explicitly state that their bodies are incomplete and name identity-based deep retrieval before reliance.

All visible structure, labels, references, and metadata count toward the rendered-token budgets.
