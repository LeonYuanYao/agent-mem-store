...
Batch shape changes the generated vectors and can change the order of close retrieval results. A different batch policy therefore requires a new adapter version and a complete replacement index. MemStore does not mix vectors produced by the previous unbounded implementation with Batch 16 vectors, and it starts a new official Shadow window after the replacement index is active.
