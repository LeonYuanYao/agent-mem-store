...
The exact first-version model remains an implementation benchmark decision. At least one lightweight and one quality-oriented candidate must be compared for recall, disk, memory, and compliance with the accepted automatic Hook deadline, currently one second under ADR-0114.
