...
MemStore Project and Memory Vault have independent ownership and lifecycles. The program lives in `AgentScratchpad/MemStore`, while the existing Obsidian vault at `<vault-root>` remains the single source of truth for personal memory data. This separation keeps personal knowledge out of the program repository by default and allows the data to be inspected, backed up, and migrated without coupling it to one implementation.
