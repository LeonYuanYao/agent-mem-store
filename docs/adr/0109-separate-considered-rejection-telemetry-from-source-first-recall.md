...
A consolidation-protected durable input omitted without represented output or
evidence-linked disposition is not accepted as a consolidation rejection: the
Worker restores that original Candidate locally. Protection requires either an
explicit user statement or at least two distinct evidence-linked importance
reasons. This fallback applies only to Candidates already admitted by
distillation and therefore does not reconstruct `no_memory`, `session_only`,
`uncertain`, or source material that Luna never proposed.
