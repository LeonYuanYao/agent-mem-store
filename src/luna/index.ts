    ...
    return this.#invokeStructured(
      ...
      governanceOutputJsonSchema,
      {
        ...
        rules: [
          ...
          "When capacityPressures is present, archive_for_capacity is the only action authorized solely by capacity pressure. capacity.eligible is necessary but never sufficient: archive only when the supplied body, applicability, provenance, and capacity signals concretely show low reuse value, transient specificity, or safely retained duplication. Cite that content-specific basis as well as the capacity signals. Never use ordinary archive to bypass capacity protection.",
          "When capacity.redundantByMemoryId is present, supersede_for_capacity may archive that exact target in favor of that exact successor. This successor-preserving action may be used when the only protection reasons are protected_importance, high_value, or multiple_evidence_sources because the reviewed successor retains the claim. Use no other successor: local validation requires a current, unambiguous, reviewed subsumption decision and records successor continuity.",
          "Do not use supersede_for_capacity when protectionReasons contains pinned, retain_forever, startup_always, human_authority, active_relationship, verification_open, review_open, recent_revision, recently_selected, or any unrecognized protection. A reviewed duplicate decision is already supplied through redundantByMemoryId and does not require auditSignals.reviewedDuplicateClusters on a capacity-only page.",
          "Do not propose archive_for_capacity for Human-authored, pinned, retain-forever, startup-always, high-value, multiply evidenced, recently revised, recently selected, related, or actively reviewed Memory. Local validation will reject the entire model result if any protected target is included.",
          "Do not treat age, zero selections, capacity.eligible, or capacity pressure by itself as proof of low value. Preserve durable workflows, failure and recovery knowledge, constraints, negations, safety rules, architecture and API contracts, and expensive-to-rediscover knowledge even when they were not recently selected.",
          "Capacity work may reduce the pressured Space toward lowWater, but there is no quota to fill. Prefer no action when the page does not establish a clearly low-value target, and never invent actions or exceed the supplied eligible page.",
