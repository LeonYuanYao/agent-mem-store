...
async function runCodexHook(arguments_: readonly string[]): Promise<void> {
  const event = z.enum(["SessionStart", "UserPromptSubmit", "PostToolUse", "Stop", "SessionEnd"])
    .parse(arguments_[2]);
  const runtimeRoot = process.env.MEMSTORE_RUNTIME_ROOT;
  if (runtimeRoot === undefined) {
    throw new MemStoreCommandError("runtime_required", "MEMSTORE_RUNTIME_ROOT is required for the Codex Hook adapter.");
  }
  const input = z.record(z.string(), z.unknown()).parse(JSON.parse(await readStandardInput()));
