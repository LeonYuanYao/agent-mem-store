...
test("Stop persists before a slow Git lookup and deferred import honors the exact session route", async () => {
  ...
  const result = spawnSync(process.execPath, ["--import", "tsx", "src/cli/hook.ts", "codex", "Stop"], {
    env: { ...process.env, PATH: `${bin}:${process.env.PATH ?? ""}`, MEMSTORE_RUNTIME_ROOT: runtimeRoot },
    encoding: "utf8", timeout: 1800, killSignal: "SIGKILL",
    input: JSON.stringify({ session_id: "deferred-stop", turn_id: "slow-git", cwd: root, last_assistant_message: "Durable fixture." })
  });
...
test("Stop reports a body-free lock failure before the host deadline", async () => {
  ...
  const result = spawnSync(process.execPath, ["--import", "tsx", "src/cli/hook.ts", "codex", "Stop"], {
    env: { ...process.env, MEMSTORE_RUNTIME_ROOT: runtimeRoot }, encoding: "utf8", timeout: 1800,
    input: JSON.stringify({ session_id: "lock-stop", turn_id: "locked", cwd: root, last_assistant_message: "private diagnostic fixture must not be printed" })
  });
  expect(result.error).toBeUndefined();
...
test("Stop recovers a transient Inbox lock within its shared budget", async () => {
  ...
  const child = spawn(process.execPath, ["--import", "tsx", "src/cli/hook.ts", "codex", "Stop"], {
    env: { ...process.env, MEMSTORE_RUNTIME_ROOT: runtimeRoot }, stdio: ["pipe", "pipe", "pipe"]
  });
  let stdout = "";
  child.stdout.setEncoding("utf8").on("data", (chunk: string) => { stdout += chunk; });
  child.stderr.resume();
  child.stdin.end(JSON.stringify({ session_id: "transient", turn_id: "transient", cwd: root, last_assistant_message: "Fixture." }));
...
syncBuiltinESMExports();\n`);
  const result = spawnSync(process.execPath, ["--import", preload, "--import", "tsx", "src/cli/hook.ts", "codex", "Stop"], {
    env: { ...process.env, MEMSTORE_RUNTIME_ROOT: runtimeRoot }, encoding: "utf8", timeout: 1950, killSignal: "SIGKILL",
    input: JSON.stringify({ session_id: "watchdog", turn_id: "watchdog", cwd: root, last_assistant_message: "Fixture." })
  });
  expect(result.error).toBeUndefined();
  expect(result.status).toBe(0);
